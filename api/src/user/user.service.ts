...
export class UserService {
  ...
  async getRoomByName(roomName: string, rooms?: Room[]): Promise<number> {
    // console.log('rooms inside', rooms)
    const roomIndex = rooms
      ? rooms.findIndex((room) => room?.name === roomName)
      : this.rooms.findIndex((room) => room?.name === roomName);
  ...
  async getRoomDetailByName(name: string): Promise<Room> {
    const idx = await this.getRoomByName(name)
